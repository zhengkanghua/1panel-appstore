...
- `7.3.0` 严格跟随当前官方 Compose，固定使用 `opencloudeu/opencloud-rolling:7.3.0`。生产镜像仓库目前没有 `7.3.0` 标签，因此没有使用不存在的 `opencloudeu/opencloud:7.3.0`。新安装优先选择此固定版本。
- `latest` 是为仓库早期已有安装保留的兼容入口，当前内容与 `7.3.0` 相同。它同样固定到 `7.3.0` 镜像而不使用浮动 Docker 标签，避免不同安装时间得到不同程序版本；两者看似重复是有意保留，不应直接删除既有安装所依赖的版本入口。
...
1Panel 当前应用表单只支持字段类型、标签、默认值、校验和安装后是否可编辑，不支持根据某个选项动态显示或隐藏其他字段。因此选择本地存储时仍会看到 S3 字段，保持默认值即可；未启用邮件通知时，SMTP 字段也保持默认值即可。表单标签已标明这些条件字段的适用范围，不使用未受 1Panel 支持的自定义条件属性。
...
Cloudflare R2 提供兼容 S3 的 API，可作为 OpenCloud `decomposeds3` 用户文件主存储。先创建专用 Bucket 和具备对象读取、写入、列出、删除权限的 R2 API Token，再在首次安装时填写：
...
| S3 区域 | `auto` |
| S3 Access Key | R2 S3 API Access Key ID |
| S3 Secret Key | R2 S3 API Secret Access Key |
| S3 存储桶 | 已创建的专用 Bucket 名称 |
| S3 事件消费者数量 | `5`（官方默认值） |

端点必须使用 R2 的 S3 API Endpoint，不能使用 `r2.dev` 公共地址或自定义下载域名。使用 R2 可以避免迁移服务器时复制大量文件对象，但 OpenCloud 并不会因此变成无状态应用：`./data/config` 和 `./data/storage` 中仍有配置、密钥、系统存储和文件元数据，必须与 Bucket 一起备份和恢复。

## 配置生效时机

在 1Panel 已安装应用的“参数”页面中，`data.yml` 的 `edit: true` 字段可修改；保存后 1Panel 会重建应用容器，期间会有短暂中断。各类配置的处理方式如下：

| 配置 | 安装后处理方式 |
| --- | --- |
| 外部访问地址、网页端口、监听地址、内置 TLS、跳过证书验证 | 可修改，重建容器后生效；同时更新 1Panel 反代域名、目标端口和证书 |
| 管理员初始密码 | 仅首次启动生效；之后在 OpenCloud 中或用官方 CLI 修改密码 |
| 容器 UID:GID、配置/数据/扩展目录 | 仅首次安装设置；变更涉及目录迁移和属主调整，不允许仅改参数 |
| 用户文件主存储、S3 端点、区域、Bucket | 仅首次安装设置；切换或迁移对象存储不能只改环境变量 |
| S3 Access Key、Secret Key | 可修改，用于同一端点和 Bucket 的凭据轮换；确认新凭据权限后再保存 |
| S3 事件消费者数量 | 可修改，重建容器后生效；没有压测依据时保持官方默认值 `5` |
| 时区、默认语言、Basic Auth、更新检查、归档下载上限 | 可修改，重建容器后生效 |
| 演示用户 | 仅首次启动生效，且生产环境必须关闭；关闭参数不会删除已经创建的演示用户 |
| 额外内置服务和全部 SMTP 参数 | 可修改，重建容器后生效；只有启用 `notifications` 时 SMTP 参数才使用 |
| 日志级别、日志格式、Docker 日志驱动 | 可修改，重建容器后生效 |
| 公共分享密码要求和密码策略 | 可修改，重建容器后对后续操作生效，不会自动修改已有用户密码或分享 |
